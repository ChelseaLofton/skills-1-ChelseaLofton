...
def sign_and_parity(number):
    ...
    if number > 0:
        about_our_number.append("Positive")
    else:
        about_our_number.append("Negative")
    if number % 2 == 0:
        about_our_number.append("Even")
    else:
        about_our_number.append("Odd")
    
    return about_our_number
...
def write_letter(recipients_name, job_title, sender_name):
    name_and_job = full_title(recipients_name, job_title)
    print(f"""Dear {name_and_job}, I think you are amazing! Sincerely, {sender_name}""")
